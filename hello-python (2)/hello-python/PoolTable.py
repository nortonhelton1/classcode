...
class Table:
  ...
  with open("check_logs.txt", "a") as file:
   file.write(str(self.table_number, self.start_time))
  ...
  while True:
    ...
    if choice == "1":
      ...
      choice = int(input("Choose a table do you want?")) 
      ...
      for table in tables:
        print(f"Table {table.table_Number}")
      choice = int(input("Choose a table do you want to check-in?"))
      table = tables[choice - 1]
      table.end_time = format_time(datetime.now())
